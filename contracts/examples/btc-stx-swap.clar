...
(define-public (submit-swap
  ...
  (let (
      (swap (unwrap! (map-get? swaps id) ERR_INVALID_ID))
      (tx-buff (contract-call? .clarity-bitcoin-helper concat-tx tx))
    ...
    (match (contract-call? .clarity-bitcoin was-tx-mined-compact height tx-buff
      ...
      result (begin
        ...
        (asserts! (not (get done swap)) ERR_ALREADY_DONE)
        (match (get out
          (unwrap! (get-out-value tx (get btc-receiver swap)) ERR_NATIVE_FAILURE)
        )
          out (if (>= (get value out) (get sats swap))
            (begin
              (map-set swaps id (merge swap { done: true }))
              (map-set submitted-btc-txs result id)
              (as-contract (stx-transfer? (get amount swap) tx-sender
  ...
  (let (
    ...
    (match (contract-call? .clarity-bitcoin was-segwit-tx-mined-compact height
      ...
      result (begin
        ...
        (asserts! (not (get done swap)) ERR_ALREADY_DONE)
        (match (get out
          (unwrap! (get-out-value wtx (get btc-receiver swap)) ERR_NATIVE_FAILURE)
        )
          out (if (>= (get value out) (get sats swap))
            (begin
              (map-set swaps id (merge swap { done: true }))
