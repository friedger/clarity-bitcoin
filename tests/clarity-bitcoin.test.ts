...
describe('Bitcoin library', () => {
  ...
  it('Ensure that buffers are correctly reversed', () => {
    ...
    let response = simnet.callReadOnlyFn(
      'clarity-bitcoin',
      'reverse-buff32',
      [Cl.bufferFromHex('74d350ca44c324f4643274b98801f9a023b2b8b72e8e895879fd9070a68f7f1f')],
      deployer
    );
    expect(response.result).toBeBuff(
      hexToBytes('1f7f8fa67090fd7958898e2eb7b8b223a0f90188b9743264f424c344ca50d374')
    );
  ...
  it('Ensure that merkle proof can be validated', () => {
    ...
    let response = verifyMerkleProof(
      // txid
      hexToBytes('cb2e23bc96049cb71489f7e98817888a927b618e9d21700120c59b4f762f16f1'),
      // reversed merkle root
      hexToBytes('c9c02be3c9d6a3d97f048d19ffff34817ffa54e7eec51bac79bca5807f64375a'),
      {
        hashes: [
          // sibling txid (in block 150000)
          hexToBytes('f1162f764f9bc5200170219d8e617b928a881788e9f78914b79c0496bc232ecb'),
  ...
  it('Ensure that merkle proof can be validated', () => {
    ...
    let response = verifyMerkleProof(
      ...
      hexToBytes('b152eca4364850f3424c7ac2b337d606c5ca0a3f96f1554f8db33d2f6f130bbe'),
      {
        hashes: [
          // reversed sibling txid (in block 150000)
          hexToBytes('ae1e670bdbf8ab984f412e6102c369aeca2ced933a1de74712ccda5edaf4ee57'),
          // 3 intermediate double-sha256 hashes
          hexToBytes('efc2b3db87ff4f00c79dfa8f732a23c0e18587a73a839b7710234583cdd03db9'),
          hexToBytes('f1b6fe8fc2ab800e6d76ee975a002d3e67a60b51a62085a07289505b8d03f149'),
  ...
  it('Ensure that merkle proof with wrong txid is detected', () => {
    ...
    let response = verifyMerkleProof(
      // CORRUPTED
      hexToBytes('25c6a1f8c0b5be2bee1e8dd3478b4ec8f54bbc3742eaf90bfb5aed46cf217ad9'),
      hexToBytes('b152eca4364850f3424c7ac2b337d606c5ca0a3f96f1554f8db33d2e6f130bbe'),
      {
        hashes: [
          hexToBytes('ae1e670bdbf8ab984f412e6102c369aeca2ced933a1de74712ccda5edaf4ee57'),
          hexToBytes('efc2b3db87ff4f00c79dfa8f732a23c0e18587a73a839b7710234583cdd03db9'),
  ...
  it('Ensure that merkle proof with wrong tree is detected', () => {
    ...
    let response = verifyMerkleProof(
      ...
      hexToBytes('b152eca4364850f3424c7ac2b337d606c5ca0a3f96f1554f8db33d2f6f130bbe'),
      {
        hashes: [
          // CORRUPTED
          hexToBytes('ae1e670bdbf8ab984f412e6102c369aeca2ced933a1de74712ccda5edaf4ee58'),
          hexToBytes('efc2b3db87ff4f00c79dfa8f732a23c0e18587a73a839b7710234583cdd03db9'),
          hexToBytes('f1b6fe8fc2ab800e6d76ee975a002d3e67a60b51a62085a07289505b8d03f149'),
          hexToBytes('e827331b1fe7a2689fbc23d14cd21317c699596cbca222182a489322ece1fa74'),
        ],
