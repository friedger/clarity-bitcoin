...
describe("Send to first input", () => {
  it("Ensure that scriptSig is converted to principal", () => {
    ...
    expect(response.result).toBeSome(
      Cl.standardPrincipal("ST2X7X1A0649S3BJR0DEB58NQ73E24FVWPPVK11WA")
    );
  });
    ...
    const block = chain.mineBlock([
      Tx.callPublicFn(
        ...
        [
          ...
          Cl.tuple({
            version: Cl.buffer(hexToBytes("00")),
            parent: Cl.buffer(hexToBytes("00")),
            "merkle-root": Cl.buffer(hexToBytes("00")),
            timestamp: Cl.buffer(hexToBytes("00")),
            nbits: Cl.buffer(hexToBytes("00")),
            nonce: Cl.buffer(hexToBytes("00")),
          }),
          Cl.tuple({
            "tx-index": Cl.uint(1),
            hashes: Cl.list([]),
